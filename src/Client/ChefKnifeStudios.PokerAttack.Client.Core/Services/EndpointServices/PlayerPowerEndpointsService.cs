...
public class PlayerPowerEndpointsService : IPlayerPowerEndpointsService
{
    ...
    public async Task<Result<PlayerPowerDTO>> SelectPlayerPowerAsync(string gameId, string playerId, string powerId, CancellationToken cancellationToken = default)
    {
        try
        {
            var res = await _httpService.GetAsync<PlayerPowerDTO>(
